...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] flex flex-col font-sans text-slate-300" id="app-root">
      
      {/* Top micro status bar */}
      <div className="bg-[#0E0E10] text-[#10b981] text-[11px] py-2 px-6 flex justify-between items-center border-b border-slate-800">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-ping" />
          <span className="font-medium tracking-tight text-slate-400">AI 智能获客增长系统 • 尊享版 (Status: Scanning Global Markets)</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="bg-[#121214] text-emerald-400 py-0.5 px-2 rounded-md font-semibold border border-slate-800">
            引擎：{apiEngine}
          </span>
          <span className="hidden sm:inline text-slate-550 font-mono">2026.06.08 实时在线</span>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col md:flex-row">
        ...
        <aside className="w-full md:w-60 bg-[#0E0E10] border-r border-slate-800 p-5 flex flex-col justify-between shrink-0">
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2.5 pb-4 border-b border-slate-800">
              <div className="w-8 h-8 rounded-lg bg-emerald-500 flex items-center justify-center text-black shadow-md shadow-emerald-500/20">
                <Compass className="w-4.5 h-4.5 animate-spin text-black" style={{ animationDuration: '8s' }} />
              </div>
              <div>
                <h1 className="text-sm font-serif italic text-white tracking-tight flex items-center gap-1">
                  ChemGrowth
                  <span className="text-[10px] text-emerald-400 font-sans uppercase font-bold not-italic">AI</span>
                </h1>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Sourcing Suite</p>
              </div>
            </div>
            ...
            <nav className="space-y-1">
              <button 
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  activeTab === 'dashboard'
                    ? 'bg-slate-800 text-white font-extrabold shadow-sm border border-slate-700/60'
              ...
              <button 
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  activeTab === 'pipeline'
                    ? 'bg-slate-800 text-white font-extrabold shadow-xs border border-slate-700/60'
                    : 'text-slate-400 hover:bg-slate-900/50 hover:text-white'
                }`}
              >
          ...
          <div className="space-y-4">
            {/* AI Credits section */}
            <div className="p-3.5 bg-emerald-950/20 border border-emerald-900/40 rounded-lg hidden md:block">
              <div className="flex justify-between items-center mb-1">
                <p className="text-[10px] uppercase tracking-wider text-emerald-400 font-extrabold">AI 诊断额度 (Credits)</p>
              ...
              <p className="text-[9px] mt-1.5 text-slate-500 font-mono">7,540 / 10,000 PINGS</p>
            </div>

            {/* Under footer profile info */}
            <div className="pt-4 border-t border-slate-800 hidden md:block">
              <div className="flex items-center gap-2.5 text-xs text-slate-400 bg-[#121214] p-3 rounded-lg border border-slate-800">
                <div className="w-7 h-7 rounded-full bg-emerald-500 flex items-center justify-center font-bold text-black font-serif">
                  W
                </div>
                <div className="truncate flex-1">
                  <span className="font-bold text-white block">张伟 (Zhang Wei)</span>
                  <span className="text-[10px] text-slate-500 truncate block">manager_sp@chemgrowth.com</span>
                </div>
              </div>
            </div>
          </div>
