...
// Helper to call generateContent with exponential backoff retry for transient errors (e.g., 503 Service Unavailable, 429 Rate Limit)
async function generateContentWithRetry(client: GoogleGenAI, params: any, maxRetries = 3, initialDelayMs = 1200) {
  let attempt = 0;
  while (true) {
    try {
      ...
      if (isTransient && attempt < maxRetries) {
        ...
        console.warn(`[Gemini API Warning] Transient error (${errorMessage}). Retrying in ${delay}ms... (Attempt ${attempt}/${maxRetries})`);
...
function getMockReport(comp: string, cty: string, web: string) {
  // Let's check if the corporate name looks suspicious (e.g. Zeta) to mock a blacklist warning
  const isSuspicious = comp.toLowerCase().includes("zeta") || comp.toLowerCase().includes("sanction");
  ...
  return {
    ...
    country: cty,
    website: web,
    businessScope: `${comp}主要经营化学品供应链代理、特纯实验室试剂、生物配方对照管线的分销配送，深耕当地终端高密研发及多晶制造网络。`,
    registrationStatus: isSuspicious 
      ? `异常存续 (Suspended/Pending Audit) | 注册于特拉华州，目前存在未结税务清算与反倾销纠纷诉讼。`
      : `核查存续 (Active/Incorporated)。注册资金：约2500万当地货币。注册纳税号合法可信。`,
    ...
    negativeNews: isSuspicious
      ...
      : `未检查到公开的反补贴倾销制裁、重大环境泄漏处罚或者欠款诉讼。但在海外地方法院有一起2024年的知识产权版权误标纠纷仲裁案，现已妥善终结。`,
    sanctionRisk: isSuspicious ? "高风险" : "低风险",
    websiteTrustScore: isSuspicious ? 45 : 88,
    emailTrustScore: isSuspicious ? 52 : 91,
    purchasePotentialScore: isSuspicious ? 30 : 84,
    creditRiskLevel: isSuspicious ? "高" : "低",
    paymentSuggestion: isSuspicious 
      ? `强烈建议拒绝赊销！若交易必须采取 100% 前 T/T (全额预付) 并在发货前审核全部出口终归。`
      : `对首单风险建议电汇 30% 到 50% 定金，发货前见提单复印件结清 (TT 30% Pre-paid + 70% against BL copia)。老客户可提单付或长期信用 L/C at Sight。`,
    recommendedProducts: [
    ...
    summary: isSuspicious
      ...
      : `${comp}是一家实力中上、资信良好的优秀本土经销商，未在官方黑名单之列，有强烈的中国高品质货源定制对接意愿，采购转化概率极高。(智能备用引擎保障)`,
    createdAt: new Date().toISOString().replace('T', ' ').substring(0, 19),
    
    // STEP 1 & 2 & 3 Fallback entries
    ehsCompliance: {
      requiredCertificates: isSuspicious
        ? ['特种易制毒剧毒化学品许可证 (DEA Form 223)', 'EPA 危险物进出口许可']
        : ['清关原产地证 (Form B)', 'IMDG 危化品安全说明书 (MSDS/SDS)', 'ISO9001 质量认证'],
      sanctionNotes: isSuspicious
        ? '⚠️ 系统重度警告：该司与被 OFAC 列入制裁清单的特种中间体转运实体存在高频关联交易嫌疑，触发二级严重合规违规。'
        : '经 OFAC 数据库和欧盟合规穿透审计：无已知化学制裁指标。其注册地及运营节点完全安全。',
      blacklistVerification: isSuspicious
        ? '🚨 联合黑名单阻断警告：系统在交叉结算校验中，匹配到该司历史往来残留早期 1.0 版本优惠券（version 1.0 coupons）抵扣异常财务欠账纠纷，交叉对账存在不可平账差额！已强制拦截通道！'
        : '联合黑名单系统自查：安全绿色通行。已拉取历史 1.0 版本的优惠券（version 1.0 coupons）及其相关商业系统记录进行交叉核验验证，无任何争议违约坏账。',
    ...
    supplyChain: {
      competitors: isSuspicious
        ...
        : [
            { name: 'Sigma-Aldo Sourcing Ltd', share: 45 },
            { name: 'Generic Chemical Dis', share: 35 },
            { name: 'Other Spot Broker', share: 20 }
          ],
      nextSourcingWindow: isSuspicious ? '已无限期阻断交易' : '2026年Q3期初 (库存即将触底，下一采购采购量约 25 吨)',
      portAnalysis: isSuspicious 
        ? '逆向追溯被阻断：因合规不符被海关边境处长期暂停自主清关权。'
        : '通过海关大数据流向分析：该司主要进口口岸清关流程高度流利通畅，在德法主要港口有专属绿色通道豁免权。'
    },
    financeRating: {
      creditRatingScore: isSuspicious ? 10 : 85,
      sinosureLimitEstimated: isSuspicious ? '$0 USD (中信保列入拒保黑名单)' : '$200,000 USD (中信保定级：A级良好)',
      baseGmvLastYear: isSuspicious ? 50000 : 500000,
      targetGmvThisYear: isSuspicious ? 100000 : 900000,
      sourcingIndexDelta: isSuspicious ? 0 : 150000,
      gmvContribution: isSuspicious ? 0.0 : 0.375, // 0.375 = 150000 / (900000 - 500000) (基于绝对差值)
      suggestedPaymentScheme: isSuspicious
        ? '中信保已拒保。不建议给予任何信用授信！务必采用 100% 前 T/T (全款预付)。'
        : '动态财险方案：建议首两笔款前 T/T (30% 预付 + 70% 提单付)，客户信誉评测结果处于中信保 $200K 承保范围内，中后期完全可提供 OA 30-45天 以反蚕食老供货商 Sigma 份额！'
    },
    milestoneStatus: isSuspicious ? 'Requirement freeze' : 'Pilot order'
  };
}
...
app.post("/api/ai/scan-leads", async (req, res) => {
  ...
  try {
            ...
            leads: {
              ...
              items: {
                ...
                required: [
                  ...
                  "leadScore", "leadGrade", "riskLevel", "urgencyLevel", "recommendedAction"
                ]
              }
            }
          },
    ...
    const withIds = (parsed.leads || []).map((lead: any) => ({
      ...
      id: genId("L"),
      source: src,
      urgencyLevel: lead.urgencyLevel || (Math.random() > 0.5 ? "高" : "中"),
      status: "待联系",
      createdAt: new Date().toISOString().replace('T', ' ').substring(0, 19)
    }));
